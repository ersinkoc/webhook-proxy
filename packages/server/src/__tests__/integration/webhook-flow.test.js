...
describe('Webhook Flow Integration', () => {
    ...
    beforeAll(async () => {
        ...
        mockServer = new test_utils_1.MockWebhookServer();
        await mockServer.start(3001);
        // Setup socket connection
        socketServer = app.io;
        socketClient = (0, socket_io_client_1.io)(`http://localhost:${config_1.config.port}`, {
            auth: {
                token: testApiKey,
            },
        });
    ...
    describe('End-to-end webhook processing', () => {
        ...
        it('should handle multiple concurrent webhooks', async () => {
            ...
            const promises = Array.from({ length: 10 }, (_, i) => app.inject({
                method: 'POST',
                url: `/webhook/${endpoint.slug}`,
    ...
    describe('Webhook management API', () => {
        it('should list webhooks with filtering', async () => {
            ...
            expect(successData.items.every((w) => w.statusCode < 400)).toBe(true);
    ...
    describe('Real-time features', () => {
        it('should broadcast webhook events to multiple clients', async () => {
            ...
            const clients = await Promise.all(Array.from({ length: 3 }, () => (0, socket_io_client_1.io)(`http://localhost:${config_1.config.port}`, {
        ...
        it('should handle client disconnection and reconnection', async () => {
            ...
            // Create client
            const client = (0, socket_io_client_1.io)(`http://localhost:${config_1.config.port}`, {
                auth: { token: testApiKey },
            });
            await new Promise((resolve) => {
                client.on('connect', resolve);
...
//# sourceMappingURL=webhook-flow.test.js.map