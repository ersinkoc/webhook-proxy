...
describe('Chaos Engineering - Infrastructure Failures', () => {
    ...
    beforeAll(async () => {
        ...
        queueService = new queue_1.QueueService(prisma);
    ...
    describe('Database failures', () => {
        ...
        it('should handle database query timeouts', async () => {
            ...
            prisma.webhook.create = jest.fn().mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve({
                id: 'timeout-webhook',
    ...
    describe('Redis failures', () => {
        ...
        it('should handle Redis cluster failover', async () => {
            ...
            const originalClient = queueService.redis;
            // Disconnect
    ...
    describe('Network partitions', () => {
        it('should handle partial network failure', async () => {
            ...
            const originalForward = app.webhookForwarder.forwardWebhook;
            let failureCount = 0;
            app.webhookForwarder.forwardWebhook = jest.fn().mockImplementation((options) => {
                failureCount++;
                if (failureCount % 3 === 0) {
                ...
                return originalForward.call(app.webhookForwarder, options);
            });
            // Send multiple webhooks
            const responses = await Promise.all(Array.from({ length: 10 }, () => app.inject({
                method: 'POST',
                url: `/webhook/${testEndpoint.slug}`,
            ...
            // Restore
            app.webhookForwarder.forwardWebhook = originalForward;
        });
    ...
    describe('Resource exhaustion', () => {
        it('should handle CPU exhaustion', async () => {
            ...
            const intervals = Array.from({ length: 10 }, () => setInterval(cpuIntensive, 10));
            try {
    ...
    describe('Cascading failures', () => {
        it('should prevent cascade failure from slow endpoints', async () => {
            ...
            const endpoints = await Promise.all(Array.from({ length: 5 }, async (_, i) => {
            ...
            const startTime = Date.now();
            const responses = await Promise.all(endpoints.map((endpoint) => app.inject({
                method: 'POST',
        ...
        it('should handle thundering herd after recovery', async () => {
            ...
            const originalForward = app.webhookForwarder.forwardWebhook;
            app.webhookForwarder.forwardWebhook = jest.fn().mockImplementation(() => {
                processed++;
                return Promise.resolve({
                    success: true,
                    statusCode: 200,
            ...
            for (let i = 0; i < 10; i++) {
                processingPromises.push((async () => {
                    while (true) {
                        ...
                        await app.webhookForwarder.forwardWebhook(data);
                    }
                })());
            }
            ...
            failureTypes.forEach((type) => {
                switch (type) {
                    ...
                    case 'redis':
                        ...
                        break;
                    case 'network':
                        originalHandlers.network = app.webhookForwarder.forwardWebhook;
                        break;
                }
            });
            ...
            for (const failureType of failureTypes) {
                ...
                switch (failureType) {
                    ...
                    case 'redis':
                        ...
                        break;
                    case 'network':
                        app.webhookForwarder.forwardWebhook = jest.fn()
                ...
                switch (failureType) {
                    ...
                    case 'redis':
                        ...
                        break;
                    case 'network':
                        app.webhookForwarder.forwardWebhook = originalHandlers.network;
                        break;
                }
                // Verify recovery
...
//# sourceMappingURL=infrastructure-failures.test.js.map