...
describe('Performance Optimization Tests', () => {
    ...
    describe('Memory profiling', () => {
        it('should not leak memory during long-running operations', async () => {
            ...
            const initialHeap = v8_1.default.getHeapStatistics();
            ...
            while (Date.now() < endTime) {
                ...
                if (iteration % 100 === 0) {
                    ...
                    const currentHeap = v8_1.default.getHeapStatistics();
            ...
            const finalHeap = v8_1.default.getHeapStatistics();
    ...
    describe('CPU optimization', () => {
        it('should process webhooks efficiently without blocking event loop', async () => {
            ...
            const webhookPromises = Array.from({ length: 100 }, (_, i) => app.inject({
                method: 'POST',
                url: `/webhook/${testEndpoint.slug}`,
                payload: {
                    index: i,
        ...
        it('should use efficient algorithms for webhook matching', async () => {
            ...
            const endpoints = await Promise.all(Array.from({ length: 100 }, async (_, i) => {
    ...
    describe('Database optimization', () => {
        it('should use efficient queries with proper indexing', async () => {
            // Create test data
            const webhooks = await Promise.all(Array.from({ length: 1000 }, async (_, i) => {
        ...
        it('should use connection pooling effectively', async () => {
            ...
            for (const concurrency of concurrencyLevels) {
                const startTime = perf_hooks_1.performance.now();
                const operations = Array.from({ length: concurrency }, () => prisma.webhook.count({
                    where: { endpointId: testEndpoint.id },
                }));
    ...
    describe('Network optimization', () => {
        it('should reuse HTTP connections efficiently', async () => {
            const mockForwarder = app.webhookForwarder;
            const connectionReuse = new Map();
            ...
            const promises = Array.from({ length: 20 }, () => app.inject({
                method: 'POST',
                url: `/webhook/${testEndpoint.slug}`,
    ...
    describe('Concurrency optimization', () => {
        it('should handle concurrent requests without performance degradation', async () => {
            ...
            for (const level of concurrencyLevels) {
                const startTime = perf_hooks_1.performance.now();
                const requests = Array.from({ length: level }, (_, i) => app.inject({
                    method: 'POST',
                    url: `/webhook/${testEndpoint.slug}`,
        ...
        it('should optimize worker thread usage for CPU-intensive tasks', async () => {
            ...
            for (const workers of workerCounts) {
                ...
                const requests = Array.from({ length: 20 }, () => app.inject({
                    method: 'POST',
                    url: `/webhook/${testEndpoint.slug}`,
